...
//@access    Public
const getProducts = asyncHandle(async (req: Request, res: Response) => {
  const pageSize = 10;
...
const deleteProduct = asyncHandle(async (req: Request, res: Response) => {
  const product = await Product.findById(req.params.id);
  if (product) {
...
// @access  Private
const createProductReview = asyncHandle(async (req: Request, res: Response) => {
  ...
  if (product) {
    ...
    product.rating =
            product.reviews.reduce((acc: any, item: any) => item.rating + acc, 0) / product.reviews.length;
...
// @access  Public
const getTopProducts = asyncHandle(async (req: Request, res: Response) => {
